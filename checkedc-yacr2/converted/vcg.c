...
AcyclicVCG(void)
{
	 ...
	 */
	DFSClearVCG(_Assume_bounds_cast<_Array_ptr<nodeVCGType>>(VCG, byte_count(0)));
	SCCofVCG(_Assume_bounds_cast<_Array_ptr<nodeVCGType>>(VCG, byte_count(0)), _Assume_bounds_cast<_Array_ptr<ulong>>(SCC, byte_count(0)), _Assume_bounds_cast<_Array_ptr<ulong>>(perSCC, byte_count(0)));
	for (scc = 1; scc <= totalSCC; scc++) {
	    if (perSCC[scc] > 1) {
		acyclic = FALSE;
		done = FALSE;
	 ...
	 */
	if (! done) {
	    RemoveConstraintVCG(_Assume_bounds_cast<_Array_ptr<nodeVCGType>>(VCG, byte_count(0)), _Assume_bounds_cast<_Array_ptr<ulong>>(SCC, byte_count(0)), _Assume_bounds_cast<_Array_ptr<ulong>>(perSCC, byte_count(0)), _Assume_bounds_cast<_Array_ptr<constraintVCGType *>>(removeVCG, byte_count(0)));
	}
    } while (! done);

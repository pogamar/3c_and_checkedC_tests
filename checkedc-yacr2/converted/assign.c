...
MaxNetsAssign(void)
{
	 ...
	 */
	Select(_Assume_bounds_cast<_Array_ptr<nodeVCGType>>(VCG, byte_count(0)), _Assume_bounds_cast<_Array_ptr<nodeHCGType>>(HCG, byte_count(0)), _Assume_bounds_cast<_Array_ptr<ulong>>(netsAssign, byte_count(0)), &netSelect, CROSSING);

	/*
	 * Assign net to a track.
	 */
	Assign(_Assume_bounds_cast<_Array_ptr<nodeVCGType>>(VCG,  count(select + 1)), _Assume_bounds_cast<_Array_ptr<ulong>>(netsAssign, byte_count(0)), netSelect);
...
Assign(_Array_ptr<nodeVCGType> VCG : count(select + 1), _Array_ptr<ulong> assign, ulong select)
{
    ...
    NoHCV(_Assume_bounds_cast<_Array_ptr<nodeHCGType>>(HCG, byte_count(0)), select, assign, _Assume_bounds_cast<_Array_ptr<ulong>>(tracksNoHCV, byte_count(0)));
...
*/
	...
	if (tracksAssign[track]) {
	    vcv = VCV(VCG, select, track, _Assume_bounds_cast<_Array_ptr<ulong>>(netsAssign, byte_count(0)));
	    if (vcv < vcvAssign) {
		vcvAssign = vcv;
		trackAssign = track;
...
BuildCostMatrix(_Array_ptr<nodeVCGType> VCG, _Array_ptr<nodeHCGType> HCG, _Array_ptr<ulong> netsAssign, _Array_ptr<ulong> CROSSING)
{
	...
	if (CROSSING[net]) {
	    ...
	    NoHCV(HCG, net, netsAssign, _Assume_bounds_cast<_Array_ptr<ulong>>(tracksNoHCV, byte_count(0)));
	    IdealTrack(channelTracks, cardTopNotPref, cardBotNotPref, &ideal);
